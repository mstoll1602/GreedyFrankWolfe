...
int nrhs, const mxArray* prhs[])
{
    ...
    const mwSize *idims =  mxGetDimensions(INMATRIX);//const int *idims =  mxGetDimensions(INMATRIX);
    if(idims[0] != idims[1])
    ...
    if(mxIsSparse(INMATRIX)) {   /* Sparse matrix handling */
        ...
        mwIndex *in_rows = mxGetIr(INMATRIX);//int *in_rows = mxGetIr(INMATRIX);
        mwIndex *in_cols = mxGetJc(INMATRIX);//int *in_cols = mxGetJc(INMATRIX);
