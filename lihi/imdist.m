...
function [D,ind_non_zero,rows_nonz,cols_nonz] = imdist(IM,R,scale)
%
...
[D,rows_nonz,cols_nonz] = mkImDist(IM(:,:,1),R,scale);
...
%%
function [D,ind2,ind1]=mkImDist(im,R,scale)
...
A = im2col_sliding(imBig,[R R]);
Ind=im2col_sliding(IndImage,[R R]);
...
% Dff2=exp(-((a-a2).^2 + eps)./scale^2);%% avoid true zeros, put eps instead
