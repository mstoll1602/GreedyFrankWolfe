...
int nrhs, const mxArray* prhs[])
{
  ...
  /* Make sure input number is sufficient */
    ...
    const mwSize* idims =  mxGetDimensions(DISTMATRIX);//const int *idims =  mxGetDimensions(DISTMATRIX);
    if(idims[0] != idims[1])
    ...
    if(mxIsSparse(DISTMATRIX)) {   /* Sparse matrix handling */
        ...
        mwIndex* distmat_rows = mxGetIr(DISTMATRIX);//int *distmat_rows = mxGetIr(DISTMATRIX);
        mwIndex* distmat_cols = mxGetJc(DISTMATRIX);//int *distmat_cols = mxGetJc(DISTMATRIX);
